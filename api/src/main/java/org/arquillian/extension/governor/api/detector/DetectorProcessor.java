...
public class DetectorProcessor {
    ...
    public boolean process(Annotation annotation) {
        ...
        try {
            final Detector detector = RefletionUtils.getAnnotationProperty(annotation, "detector", Detector.class);
            final Strategy strategy = detector.strategy().newInstance();
            ...
            if (strategy instanceof BaseStrategy) {
                ((BaseStrategy) strategy).detectables(detectables);
            }
